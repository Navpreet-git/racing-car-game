...
function createGameCanvas(player) {
    ...
    carImg.onload = () => {
        ...
        function generateObstacles() {
            const obstacleTypes = ['images/obstacles/landMine.png', 'images/obstacles/splash1.png', 'images/obstacles/splash2.png', 'images/obstacles/splash3.png'];
            const obstacleCount = 3; 
            for (let i = 0; i < obstacleCount; i++) {
                const type = obstacleTypes[Math.floor(Math.random() * obstacleTypes.length)];
                const x = Math.random() * (canvas.width / 3) + (canvas.width - canvas.width / 3) / 2;
        ...
        function checkForCollisions() {
            obstacles.forEach(obstacle => {
                if (
                    playerX < obstacle.x + 50 &&
                    playerX + 50 > obstacle.x &&
                    playerY < obstacle.y + 50 &&
                    playerY + 50 > obstacle.y
                ) {
                    isControlling = false;
                    displayObstacleMessage(obstacle.type);
                }
            });
        }

        function displayObstacleMessage(obstacleType) {
            ...
            if (obstacleType === 'landMine.png') {
                message.textContent = 'You hit a land mine!';
            } else {
                message.textContent = 'You hit a splash obstacle!';
            }
        ...
        function drawObstacles() {
            obstacles.forEach(obstacle => {
                ...
                obstacleImg.src = `/images/obstacles/${obstacle.type}`;
                obstacleImg.onload = () => {
                    ctx.drawImage(obstacleImg, obstacle.x, obstacle.y, 50, 50);
                };
            });
        }
